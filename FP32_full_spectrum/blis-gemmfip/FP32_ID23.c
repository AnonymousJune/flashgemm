...
int main()
{
	...
	FILE *fp;
	if ((fp = fopen("../result/blis-gemmfip.txt", "w")) == NULL)
	{
	...
	for (j = 0; j < 23; j++)
	{
		...
		printf("BLIS:  M= %-10d N=%-10d K=%-10d flops = %-10.3lf effic= %.3lf %\n",
					 M, N, K, ops / cost, ops / cost / (PEAK_GFLOPS * 64) * 100 / NUM);
		// fprintf(fp, "BLIS:  M= %-10d N=%-10d K=%-10d flops = %-10.3lf effic= %.3lf %\n",
		// 		M, N, K, ops / cost, ops / cost );
		fprintf(fp, "%.3lf \n", ops / cost);
