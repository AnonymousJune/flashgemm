...
int main()
{
	...
	FILE *fp;
	if ((fp = fopen("../result/bf16_3_openblas.txt", "w")) == NULL)
	{
	...
	for (j = 0; j < 10; j++)
	{
		...
		uint16_t *C1_f16 = (uint16_t *)malloc(M1 * N * sizeof(uint16_t));
		uint16_t *C2_f16 = (uint16_t *)malloc(M2 * N * sizeof(uint16_t));
		...
		for (i = 0; i < loop; i++)
		{
			cblas_sbgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, M1, N, K1,
									 1.0, A1, K1, B, N, 0.0, C1_f32, N);
			// f32_matrix_to_bf16_matrix(C1_f32, C1_f16, M1, N);
			cblas_sbgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, M2, N, K2,
									 1.0, A2, K2, C1_f16, N, 0.0, C2_f32, N);
			// f32_matrix_to_bf16_matrix(C2_f32, C2_f16, M1, N);
			cblas_sbgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, M3, N, K3,
		...
		printf("openblas_bf16_3:  N=%-10d M1= %-10d K1=%-10d M2= %-10d K2=%-10d M3= %-10d K3=%-10d flops = %-10.3lf effic= %.3lf %\n",
