...
int main()
{
	...
	FILE *fp;
	if ((fp = fopen("../result/bf16_openblas.txt", "w")) == NULL)
	{
	...
	for (j = 0; j < 20; j++)
	{
		...
		for (i = 0; i < loop; i++)
		{
			cblas_sbgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, M, N, K,
		...
		printf("openblas_bf16:  M= %-10d N=%-10d K=%-10d flops = %-10.3lf effic= %.3lf %\n",
