...
void matmul_example(dnnl::engine::kind engine_kind)
{
    ...
    FILE *fp;
    if ((fp = fopen("./int8_onednn.txt", "w")) == NULL)
    {
    ...
    for (int j = 0; j < 10; j++)
    {
        ...
        std::vector<uint8_t> B_data(product(B_dims));
        std::vector<uint8_t> C1_data(product(C1_dims));
        std::vector<uint8_t> C2_data(product(C2_dims));
        ...
        auto B_md = memory::desc(B_dims, dt::u8, tag::ab);
        auto C1_md = memory::desc(C1_dims, dt::u8, tag::ab);
        auto C2_md = memory::desc(C2_dims, dt::u8, tag::ab);
        ...
        fprintf(fp, "%.3lf\n", ops / cost / (PEAK_GFLOPS * 32 * 4 * 2) * 100 / NUM);
    }
    fclose(fp);
}
