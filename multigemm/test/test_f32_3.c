...
int main()
{
	...
	for (int j = 0; j < 10; j++)
	{
		...
		if (flag)
		{
			printf("f32:  N=%-6d M1= %-6d K1=%-6d M2= %-6d K2=%-6d M3= %-6d K3=%-6d flops = %-10.3lf effic= %.3lf %\n",
						 N, M1, K1, M2, K2, M3, K3, ops / cost, ops / cost / (PEAK_GFLOPS * 16 * 2 * 2) * 100 / NUM);
			fprintf(fp, "%.3lf\n", ops / cost);
		}
		else
		{
			printf("f32:  N=%-6d M1= %-6d K1=%-6d M2= %-6d K2=%-6d M3= %-6d K3=%-6d error!!!\n",
						 N, M1, K1, M2, K2, M3, K3);
			fprintf(fp, "error! \n");
		}
